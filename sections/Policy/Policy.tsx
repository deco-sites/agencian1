    ...
    <>
      <div class={`n1-policy md:n1-container md:px-[120px] text-[#ffffff]`}>
        <div
            ...
            {title && (
                <div
                    class={clx(`[&_*]:font-noto-sans [&_*]:text-[14px] [&_*]:leading-[25.2px]`)}
            ...
            {accordion && (
                <div
                    ...
                    { accordion && accordion.length > 0 && accordion.map(( item , index)=>{                        
                        return(
                            <>
                                {item.title && (
                                    <details open={index === 0 && accordionOpen ? true : item.allAccordionOpen ? true : false}>
                                        <summary 
                                            class={clx(`
                                        ...
                                        {item.description && (
                                            <div
                                                class={clx(`                                                    
                                                    ...
                                                    ${item?.existsFontArchimoto ? 'exists-archimoto' : ''}
                                                    ${item.existsBulletListColor ? 'exists-bullet' : '' }
                                                `)}
                                        ...
                                        {item?.table && item.table.thead && item.table.thead?.length > 0 && !item.disableTable && (
                                            <div 
                                                class={clx(`overflow-auto md:overflow-hidden px-[20px]`)}>
                                                ...
                                                {item?.descriptionAfterTable && (
                                                    <div
                                                        ...
                                                        dangerouslySetInnerHTML={{ __html: item.descriptionAfterTable }}
                                                    >
                                                    </div> 
                                                )}
                                            </div>
