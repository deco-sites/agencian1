...
export default function BenefitsList({ title, marginBottom, marginTop, lists,icon }: Props) {

    return (
       
       <div class=" relative w-full n1-custom-bg-img md:n1-custom-bg-img-2" style={{ marginTop: `${marginTop}`, marginBottom: `${marginBottom}` }} >
          <div
        ...
        >   
                ...
                {lists.map((list, index) => (
                    <div class="flex justify-center items-center gap-5 n1-custom-card-benefits py-4 px-4">
                        <div class="flex justify-center items-center gap-4 w-full max-x-[285px]">
                            <div  class="w-9 h-9">
                                 <Icon id="Check" size={36} strokeWidth={2} class=" text-center" /> 
                            </div>
                          
                            <div class="text-14 text-[#F3F4f7] font-normal font-noto-sans !leading-[140%]"
           ...
           {icon && (
             <Image src={icon} width={200} height={200} alt="icon" class="absolute -top-20 left-0 hidden md:block" loading="lazy" />
           )}
        </div>
       </div> 
    )
