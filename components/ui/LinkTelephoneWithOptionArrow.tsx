...
function LinkTelephoneWithOptionArrow( {text, telephone, activeArrow, width, customClass, fontSize}:Props ){
    ...
    return(
        <a 
            ...
            class={clx(`${width ? `mobile:my-0 mobile:mx-auto` : 'mobile:w-[130px]' } 
                ...
                text-16 uppercase bg-accent n1-btn-header-item--rounded border-none w-[142px] ml-[40px] ${customClass ? customClass : ''}`)}
            style={{ width : `${width ? width + 'px' : '130px'}`}}
        >            
            <span 
                class={`${fontSize ? 'text-16' : 'mobile:text-14'} text-primary text-16 mobile:text-14 font-archimoto-medium pt-[3px] !mobile:mr-[25px] font-black`}
