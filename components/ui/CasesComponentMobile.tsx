...
function CasesComponentMobile({ settingsImage }:Props){
    ...
    return(
        <>
            <div class="n1-cases-card__mobile tablet:mt-[64px] tablet:max-w-full tablet:w-full tablet:grid tablet:col-[1_/_2_span]" id={id}>
                <Slider class="n1-cases-card__carousel carousel carousel-center w-full flex py-[0] tablet:w-full gap-x-[11px] tablet:gap-x-[18px]">           
                    {settingsImage && settingsImage.length > 1 && settingsImage.map(({
                        ...
                        return(
                            <>
                                <Slider.Item index={index} class="n1-cases-card__item carousel-item  mobile:w-[95%] tablet:w-[31.666%] w-full">
                                    ...
                                    >
                                        ...
                                        <div 
                                            class={clx(`n1-cases-card__item--hover w-full absolute top-0 left-0 duration-300 
                                        ...
                                        >
                                            <div class="flex flex-col justify-between h-full pt-[38px] pb-[24px] px-[18px]">                
                                                {imageLogoHover && (
                                                    <img src={`${imageLogoHover}`} class="w-full h-auto n1-cases-card__image duration-200" alt={alt ? alt : "Logo"}
                                                        style={{ maxWidth : `${widthLogoHover ? widthLogoHover + 'px' : '346px' }` }}
                                                    />
                                                )}
                                                ...
                                                <div class="grid grid-cols-[repeat(2,_auto)] justify-between items-end">
                                                    <div class="n1-cases__info">
                                                        <div class="flex flex-col">
                                                            {infoPorcentagem && (
                                                                <span class="font-archimoto-medium text-60 text-[#ffffff] font-black">{ infoPorcentagem }</span>
                                                            )}
                                                            {infoText && (
                                                                <span class="font-archimoto-medium text-25 text-[#ffffff] font-black uppercase">{ infoText }</span>
                                                            )}
                                                        </div>

                                                        <div class={clx(`n1-cases-card__tags grid grid-cols-[repeat(3,1fr)] tablet:grid-cols-[repeat(2,1fr)] 
                                                    ...
                                                    {iconHover && (
                                                        <div class="flex justify-end w-[42px] n1-cases__icon-mobile">
                                                            <img src={`${iconHover}`} alt={alt ? alt : "Icon"} />
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
