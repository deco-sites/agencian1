...
export default function Result_achievements({
    ...
    return (
        <div class={`w-full max-w-[1200px] m-auto px-5 lg:py-0 `} style={{marginTop:`${marginTop}`, marginBottom:`${marginBottom}` }}>
            {title && (
                <div
            ...
            <div class="">
                <ul class=" gap-4 items-center grid  grind-cols-1 lg:grid-cols-2 ">
                    {cards && cards.map((card, index) => (
                        <li key={index} class="flex flex-col lg:flex-row items-start gap-4 mb-6 py-5 px-5 n1-custom-cards-description w-full lg:max-w-[580px] lg:h-[215px]">
                            {card.Iconcheck && (
                                <Image
                                    src={card.Iconcheck}
                                    alt="tags"
